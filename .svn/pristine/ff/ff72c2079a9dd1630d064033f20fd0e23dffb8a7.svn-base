...
public class SQLHelper {
    ...
    public static int getCount(final String sql, final Connection connection,
    							...
    							final BoundSql boundSql, String countBy, String filterOrderBy, Log log) throws SQLException {
		...
		if("oracle".equals(dbName)){
			if("id".equals(countBy)){
				if(BooleanType.TRUE.equals(isFilterOrderBy)){
					countSql = "select count(id) from (" + removeOrders(sql) + ") tmp_count";
				}else{
					countSql = "select count(id) from (" + sql + ") tmp_count";
				}
			}else{
				if(BooleanType.TRUE.equals(isFilterOrderBy)){
					countSql = "select count(" + countBy + ") from (" + removeOrders(sql) + ") tmp_count";
				}else{
					countSql = "select count(" + countBy + ") from (" + sql + ") tmp_count";
				}
			}
		}else{
			if("id".equals(countBy)){
