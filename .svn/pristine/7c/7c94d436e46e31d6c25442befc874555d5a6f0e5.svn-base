...
public final class EXOCREngine{
	...
	private static Logger logger = LoggerFactory.getLogger(EXOCREngine.class);
	...
	public static synchronized int init(byte [] bPath){
		URL url = EXOCREngine.class.getClassLoader().getResource("/web.xml");  
		String path = url.getPath().replace("/classes/web.xml", "");
		logger.debug("EXOCREngine init() path: " + path);
		System.load(path + "/ocr/src/libexocrengine.so");
