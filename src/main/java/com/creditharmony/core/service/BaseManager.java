...
public abstract class BaseManager {
				...
				for (String oa : StringUtils.split(orgAlias, ",")){
					if (!dataScope.contains(r.getDataScope()) && StringUtils.isNotBlank(oa)){
						if (Role.DATA_SCOPE_ALL.equals(r.getDataScope())){
							...
							sqlString.append(" OR " + oa + ".id = '" + user.getDepartment().getId() + "'");
							sqlString.append(" OR " + oa + ".parent_ids LIKE '" + user.getDepartment().getParentIds() + user.getDepartment().getId() + ",%'");
						}
						else if (Role.DATA_SCOPE_ORG.equals(r.getDataScope())){
							sqlString.append(" OR " + oa + ".id = '" + user.getDepartment().getId() + "'");
						}
						else if (Role.DATA_SCOPE_CUSTOM.equals(r.getDataScope())){
							sqlString.append(" OR EXISTS (SELECT 1 FROM gl.t_gl_role_org WHERE role_id = '" + r.getId() + "'");
	 ...
	 * @param sqlMapKey sqlMap的键值，例如设置“dsf”时，调用方法：${sqlMap.dsf}
	 * @param orgWheres org表条件，组成：部门表字段=业务表的部门字段
	 * @param userWheres user表条件，组成：用户表字段=业务表的用户字段
	 * @example
	 * 		dataScopeFilter(user, "dsf", "id=a.department_id", "id=a.create_by");
	...
	public static void dataScopeFilter(BaseEntity<?> entity, String sqlMapKey, String orgWheres, String userWheres) {
		...
		for (String where : StringUtils.split(orgWheres, ",")){
			if (Role.DATA_SCOPE_COMPANY_AND_CHILD.equals(dataScopeString)){
				// 包括本公司下的部门 （type=1:公司；type=2：部门）
				sqlString.append(" AND EXISTS (SELECT 1 FROM gl.t_gl_org");
				...
				sqlString.append(" AND EXISTS (SELECT 1 FROM gl.t_gl_org");
				sqlString.append(" WHERE (id = '" + user.getDepartment().getId() + "'");
				sqlString.append(" OR parent_ids LIKE '" + user.getDepartment().getParentIds() + user.getDepartment().getId() + ",%')");
				sqlString.append(" AND " + where +")");
				...
				sqlString.append(" AND EXISTS (SELECT 1 FROM gl.t_gl_role_org ro123456, gl.t_gl_org o123456");
		...
		for (String where : StringUtils.split(userWheres, ",")){
			if (Role.DATA_SCOPE_SELF.equals(dataScopeString)){
				sqlString.append(" AND EXISTS (SELECT 1 FROM gl.t_gl_user");
