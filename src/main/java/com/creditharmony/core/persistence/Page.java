...
public class Page<T> implements Serializable{
	...
	private static final long serialVersionUID = 6888511681509528371L;
	...
	/**
	 * select count语句的count数据库列的名称
	 */
	private String countBy = "id";
	/** 过滤orderBy,"true"-过滤掉order by; "false"-不过滤order by **/
	private String filterOrderBy = "true";
	...
	public String toString() {
		...
		if (pageNo == first || first == 0) {// 如果是首页
			sb.append("<li class=\"disabled\"><a href=\"javascript:\">首页</a></li>\n");
			sb.append("<li class=\"disabled\"><a href=\"javascript:\">&#171; 上一页</a></li>\n");
		} else {
			sb.append("<li><a href=\"javascript:\" onclick=\""+funcName+"("+first+","+pageSize+",'"+funcParam+"');\"> 首页</a></li>\n");
		...
		if (pageNo == last || last == 0 ) {
			...
			sb.append("<li><a href=\"javascript:\" onclick=\""+funcName+"("+next+","+pageSize+",'"+funcParam+"');\">"
					+ "下一页 &#187;</a></li>\n");
			sb.append("<li><a href=\"javascript:\" onclick=\""+funcName+"("+last+","+pageSize+",'"+funcParam+"');\"> 尾页</a></li>\n");
		}

		sb.append("<li class=\"\"><a href=\"javascript:\" style=\"padding-top: 1px;padding-bottom: 1px;\">当前 ");
		sb.append("<input type=\"text\" style=\"width:40px;height:100%;\" value=\"" + pageNo + "\" onkeypress=\"var e=event||window.event||this;var c=e.keyCode||e.which;if(c==13)");
		sb.append(funcName+"(this.value,"+pageSize+",'"+funcParam+"');\" onclick=\"this.select();\"/> / "+last);
...
//		sb.append("共 " + count + " 条"+(message!=null?message:"")+"</a></li>\n");
	    ...
	    String pageSizeCount = "10,20,30,40,50,80,100,150,200,300,500,800";
		
		sb.append("，每页 <select style=\"width:58px;\" value=\""+pageSize+"\"");
		sb.append(" onchange=\"" +funcName+"("+1+",this.value,'"+funcParam+"');\" > ");
		for(String countN:pageSizeCount.split(",")){
			sb.append("<option value='"+countN+"' "+(countN.equals(pageSize+"")?"selected":"") +"  >"+countN+"</option>");
		}
		sb.append("</select>条数据，");
