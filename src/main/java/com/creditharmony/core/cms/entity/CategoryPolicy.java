...
/**
 * 栏目Entity
 * @Class Name CategoryPolicy
 * @author 张岩
 * @Create In 2017年3月21日
 */
public class CategoryPolicy extends TreeEntity<CategoryPolicy> {

    public static final String DEFAULT_TEMPLATE = "frontList";

	private static final long serialVersionUID = 1L;
	private Site site;					// 归属站点
	private Org org;					// 归属部门
	...
	private List<CategoryPolicy> childList = Lists.newArrayList(); 	// 拥有子分类列表
	
	@SuppressWarnings("unused")
    private transient String moduleName;     //栏目模型名称
	...
	public CategoryPolicy(String id){
		this();
		this.id = id;
	}
	
	public CategoryPolicy(String id, Site site){
		this();
		this.id = id;
		this.setSite(site);
	}
	...
	public CategoryPolicy getParent() {
		return parent;
	}

	public void setParent(CategoryPolicy parent) {
		this.parent = parent;
	}
...
//	}
	...
	public List<CategoryPolicy> getChildList() {
		return childList;
	}

	public void setChildList(List<CategoryPolicy> childList) {
	...
	public static void sortList(List<CategoryPolicy> list, List<CategoryPolicy> sourcelist, String parentId){
		for (int i=0; i<sourcelist.size(); i++){
			CategoryPolicy e = sourcelist.get(i);
			if (e.getParent()!=null && e.getParent().getId()!=null
					&& e.getParent().getId().equals(parentId)){
				list.add(e);
				// 判断是否还有子节点, 有则继续获取子节点
				for (int j=0; j<sourcelist.size(); j++){
					CategoryPolicy child = sourcelist.get(j);
    ...
    public String getModuleName() {
        return DictUtils.getDictLabel(module,
                DictTypeIndex.CMS_MODULE_TYPE, "公共模型");
    }
